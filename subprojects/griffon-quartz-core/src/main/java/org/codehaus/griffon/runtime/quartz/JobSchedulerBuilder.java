...
public final class JobSchedulerBuilder {
    ...
    public void schedule(@Nonnull Scheduler scheduler) throws SchedulerException {
        if (cronExpression == null && trigger == null) {
            throw new IllegalStateException(format("Impossible to schedule Job '%s' without cron expression",
                jobClass.getName()));
        }
        if (cronExpression != null && trigger != null) {
            throw new IllegalStateException(format("Impossible to schedule Job '%s' with cron expression " +
